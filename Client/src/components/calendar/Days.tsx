...
const Days: React.FC<{ currentDate: Date; events: Array<any>; }> = ({ currentDate, events }) => {
    ...
    const calculateOverlaps = (eventsForDay: OverlapEventProps[]) => {
        ...
        const sortedEvents: OverlapEventProps[] = [...eventsForDay].sort((a, b) => a.start.getTime() - b.start.getTime());
        ...
        <View className='flex-1 flex-row w-full'>
            ...
            <View className='flex-1 flex-row justify-between' onLayout={onLayout}>
                {Array.from({ length: weekDays }).map((_, index) => {
                    ...
                    return (
                        <View key={index} className='flex-1 items-center pt-2 border-l border-gray-200 z-10'>
