...
function getExpoDevServerIp(): string | null {
  if (__DEV__) {
    const manifest =
      Constants.manifest || Constants.manifest2?.extra?.expoGo?.debuggerHost;
    if (manifest) {
...
async function testServerConnection(): Promise<boolean> {
  try {
    ...
    console._warn(
      `[Log Error]: Server (${LOG_SERVER_IP}) is not reachable`,
      error,
    );
    return false;
  }
...
(async function initializeCustomLogger() {
  ...
  if (serverReachable) {
    ...
    console.log(
      "[Log Info]: Using native console methods as server is unavailable",
    );
  }
})();
