...
const fetchCanteens = async (setCanteenNames: (canteens: CanteenProps[]) => void) => {
    try {
        const lastFetchTimeCanteen = await AsyncStorage.getItem("lastFetchTimeCanteen");
...
const fetchCanteenDishes = async (canteenShortName: string, setMenu: (menu: MenuData | null) => void) => {
    try {
        const response = await axiosInstance.get<MenuData>(`/canteen/${canteenShortName}/menu/all`);
