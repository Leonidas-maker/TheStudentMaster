...
async def get_university_by_id(db: AsyncSession, university_uuid: uuid.UUID) -> Optional[m_calendar.University]:
    ...
    res = await db.execute(
        select(m_calendar.University).filter(m_calendar.University.university_uuid == university_uuid)
    )
    return res.scalar_one_or_none()


async def university_exists(db: AsyncSession, university_uuid: uuid.UUID) -> bool:
    ...
    res = await db.execute(
        select(
            exists(
                ...
                .filter(m_calendar.University.university_uuid == university_uuid)
            )
        )
    )
...
async def get_university_free_rooms(
    db: AsyncSession, university_uuid: uuid.UUID, start_time: datetime.datetime, end_time: datetime.datetime
