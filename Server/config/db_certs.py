...
def generate_client_certificate(private_key, issuer_name, subject_name):
    subject = issuer_name = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.LOCALITY_NAME, "Mannheim"),
...
def generate_client_cert(console: Console):
    ...
    if ENVIRONMENT == "prod":
        ...
        if os.path.exists(client_cert_path) and os.path.exists(client_key_path):
            console.log(
                "Running in dev mode, client certificate already exists. Skipping generation.", style="bold yellow"
            )
            return
    ...
    client_cert_pem = generate_client_certificate(
        private_key, issuer_name="TheStudentMaster", subject_name="TheStudentMasterBackend"
    )
